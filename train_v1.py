...
def calculate_class_weight(dataset, class_label, num_classes):
    # Count the occurrences of the class in the dataset
    class_count = sum(1 for _, label in dataset if label == class_label)
    
    # Calculate the weight as the inverse of the class frequency
...
def train(args):
    ...
    optimizer = SGD(params=model.parameters(), lr=cfg.LEARNING_RATE, weight_decay=cfg.WEIGHT_DECAY)

    # Initialize our optimizer and loss function
    class_weights = [calculate_class_weight(train_dataloader.dataset, class_label, num_classes) for class_label in range(num_classes)]
    class_weights = torch.FloatTensor(class_weights).to(device)

    # Print class weights
    for class_label, weight in enumerate(class_weights):
        print(f"Class {class_label}: Weight = {weight.item()}")
    ...
    if device == "cuda":
        model = model.to("cpu")
    torch.save(model.state_dict(), f"checkpoints/{args.model}.pt")
